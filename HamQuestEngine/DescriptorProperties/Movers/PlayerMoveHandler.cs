        ...
        public void DoMove(Creature theCreature, ref int nextColumn, ref int nextRow,Game theGame)
        {
            ...
            else if (theCreature.Map[nextColumn][nextRow].ItemIdentifier != String.Empty)
            {
                ...
                if (itemDescriptor.GetProperty<string>(GameConstants.Properties.ItemType) == GameConstants.ItemTypes.Door)
                {
                    ...
                    bool unlocked = false;
                    foreach (string identifier in itemUnlocks.Values)
                    {
                        if (playerDescriptor.GetItemCount(identifier) >= itemUnlocks[identifier])
                        {
                            playerDescriptor.RemoveItems(identifier, itemUnlocks[identifier]);
                            unlocked = true;
                ...
                {
                    if (itemDescriptor.HasProperty(GameConstants.Properties.ItemBlocks) && playerDescriptor.HasItem(itemDescriptor.GetProperty<string>(GameConstants.Properties.ItemBlocks)))
                    {
                ...
                else if (itemDescriptor.GetProperty<string>(GameConstants.Properties.ItemType) == GameConstants.ItemTypes.Shop)
                {
                    ...
                    else
                    {
                        ...
                        string shopBehavior = itemDescriptor.GetProperty<string>(GameConstants.Properties.ShopBehavior);
                        switch (shopBehavior)
                        {
                            case GameConstants.ShopBehaviors.Vanish:
                                theCreature.Map[nextColumn][nextRow].ItemIdentifier = String.Empty;
                                break;
                            case GameConstants.ShopBehaviors.Teleport:
                                playerDescriptor.Maze.RespawnItem(theCreature.Map[nextColumn][nextRow].ItemIdentifier);
                                theCreature.Map[nextColumn][nextRow].ItemIdentifier = String.Empty;
                                break;
                            case GameConstants.ShopBehaviors.Persist:
                                nextColumn = theCreature.Column;
                                nextRow = theCreature.Row;
                                break;
                        }
                ...
                {
                    bool success = false;
                    if (playerDescriptor.HasItem("Amulet"))
                    {
                    ...
                    else if (playerDescriptor.QuestItems == theGame.TableSet.ItemTable.QuestItemCount && playerDescriptor.HasItem("RainbowKey"))
                    {
