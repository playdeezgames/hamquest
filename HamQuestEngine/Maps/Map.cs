...
namespace HamQuestEngine
{
    ...
    public class Map:Board<MapCell>,IGameClient
    {
        ...
        private string[] GetMapFileList(MazeCellBase<Directions, int, Portal, CellInfo> cell)
        {
        ...
        public void GenerateTerrain(MazeCellBase<Directions, int, Portal, CellInfo> mazeCell)
        {
            ...
            foreach (string fileName in fileList)
            {
                if (cqm == null)
                {
                    cqm = CQMLoader.LoadFromFile("./maps/" + fileName);
                }
                else
                {
                    cqm.Blend(CQMLoader.LoadFromFile("./maps/" + fileName), 0, 0, 255);
                }
            }
            int column;
            int row;
        ...
        public void Generate(MazeCellBase<Directions, int, Portal, CellInfo> mazeCell)
        {
            GenerateTerrain(mazeCell);
            int column;
            int row;
            foreach (string itemIdentifier in mazeCell.CellInfo.Items.Values)
            {
            ...
            foreach (string creatureIdentifier in mazeCell.CellInfo.Creatures.Values)
            {
