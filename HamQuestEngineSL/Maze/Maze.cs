...
{
    public class Maze:MazeBase<Directions,Portal,CellInfo>,IGameClient
    {
        private Game game;
        public Game Game
        {
        ...
        private void populateMaze()
        {
            ...
            List<MazeCellBase<Directions, Portal, CellInfo>> deadEndCells = new List<MazeCellBase<Directions, Portal, CellInfo>>();
            List<MazeCellBase<Directions, Portal, CellInfo>> nonDeadEndCells = new List<MazeCellBase<Directions, Portal, CellInfo>>();
            for (column = 0; column < Columns; ++column)
            {
                for (row = 0; row < Rows; ++row)
                {
                    MazeCellBase<Directions, Portal, CellInfo> cell = this[column][row];
                    int openCount = 0;
            ...
            foreach (MazeCellBase<Directions, Portal, CellInfo> cell in deadEndCells)
            {
                for (direction = 0; direction < directions.Count; ++direction)
                {
            ...
            foreach (int lockType in lockCounts.Keys)
            {
                ...
                {
                    count--;
                    MazeCellBase<Directions, Portal, CellInfo> cell = nonDeadEndCells[Game.RandomNumberGenerator.Next(nonDeadEndCells.Count)];
            ...
            : base(theGame.TableSet.PropertyGroupTable.GetPropertyDescriptor(GameConstants.PropertyGroups.MazeDimensions).GetProperty<int>(GameConstants.Properties.Columns), theGame.TableSet.PropertyGroupTable.GetPropertyDescriptor(GameConstants.PropertyGroups.MazeDimensions).GetProperty<int>(GameConstants.Properties.Rows))
        {
            ...
            OnPostGenerate += new MazeBase<Directions, Portal, CellInfo>.MazeGenerationDelegate(this.populateMaze);
        }
